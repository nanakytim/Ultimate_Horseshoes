package net.nanaky.ultimate_horseshoes.config;
...
public class ModConfigs {
    ...
    private static final Path CONFIG_PATH = FabricLoader.getInstance()
            .getConfigDir().resolve("ultimate_horseshoes.json");
    ...
    public static void registerConfigs() {
        ...
        if (Files.exists(CONFIG_PATH)) {
            try (Reader r = Files.newBufferedReader(CONFIG_PATH)) {
                ...
                System.err.println("[Ultimate Horseshoes] Failed to load config, using defaults.");
                json = defaults();
            }
        } else {
            json = defaults();
            try (Writer w = Files.newBufferedWriter(CONFIG_PATH)) {
                ...
                System.out.println("[Ultimate Horseshoes] Config generated at " + CONFIG_PATH);
            } catch (Exception e) {
                System.err.println("[Ultimate Horseshoes] Failed to write config.");
            }
        }

        HORSESHOES_DURABILITY           = getBoolean(json, "ultimate_horseshoes_durability",           true);
        HORSESHOES_DURABILITY_THRESHOLD = getInt    (json, "ultimate_horseshoes_durability_threshold",  7);
    ...
    public static void save() {
        try (Writer w = Files.newBufferedWriter(CONFIG_PATH)) {
            ...
            System.err.println("[Ultimate Horseshoes] Failed to save config.");
        }
    }

    private static JsonObject defaults() {
        JsonObject o = new JsonObject();
        o.addProperty("ultimate_horseshoes_durability",           true);
        o.addProperty("ultimate_horseshoes_durability_threshold", 7);
